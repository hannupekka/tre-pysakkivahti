...
public class AddStopActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mAdapter = new SuggestionAdapter(this, R.layout.stoplist_item);

		// Get reference to search box
		mNewStop = (AutoCompleteTextView) findViewById(R.id.new_stop);
		
		// Get reference to progressbar
		mLoading = (ProgressBar) findViewById(R.id.new_stop_loading);
		mLoading.setVisibility(View.GONE);
		
		// Set minimum char count for search and set adapter
		mNewStop.setThreshold(4);
		...
		mAddStop = (Button) findViewById(R.id.save_new_stop);
		mAddStop.setEnabled(false);
		mClearStop = (Button) findViewById(R.id.clear_new_stop);
		mClearStop.setEnabled(false);
			...
			public void onClick(View v) {
				...
				if (mSuggestedStop != null) {
					...
					if (!db.stopExists(mSuggestedStop)) {
						...
						Toast.makeText(getApplicationContext(), getResources().getString(R.string.stop_added), Toast.LENGTH_SHORT).show();
					} else {
						Toast.makeText(getApplicationContext(), getResources().getString(R.string.stop_exists), Toast.LENGTH_SHORT).show();
					}
					// Disconnect from DB
					db.close();
